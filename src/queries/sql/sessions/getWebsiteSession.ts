...
async function relationalQuery(websiteId: string, sessionId: string) {
  ...
  return rawQuery(
    ...
    group by session.session_id, session.distinct_id, visit_id, session.website_id, session.browser, session.os, session.device, session.screen, session.language, session.country, session.region, session.city, session.encrypted_ip) t
    group by id, distinct_id, website_id, browser, os, device, screen, language, country, region, city, encrypted_ip;
    `,
    { websiteId, sessionId },
